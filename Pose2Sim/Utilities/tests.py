...
'''
    ...
    python tests.py
        OR
    from PoseAnalysis.Utilities.tests import TestWorkflow; TestWorkflow.test_workflow(mock_input='no')
'''
...
class TestWorkflow(unittest.TestCase):
    ...
    def test_workflow(self, mock_input):
        ...
        # Step by step
        PoseAnalysis.calibration(config_dict)
        PoseAnalysis.poseEstimation(config_dict)
        PoseAnalysis.synchronization(config_dict)
        PoseAnalysis.personAssociation(config_dict)
        PoseAnalysis.triangulation(config_dict)
        PoseAnalysis.filtering(config_dict)
        PoseAnalysis.markerAugmentation(config_dict)
        PoseAnalysis.kinematics(config_dict)
        ...
        PoseAnalysis.runAll(config_dict)
        ...
        PoseAnalysis.calibration(config_dict)
        PoseAnalysis.poseEstimation(config_dict)
        # PoseAnalysis.synchronization(config_dict) # No test for synchronization for multi-person
        PoseAnalysis.personAssociation(config_dict)
        PoseAnalysis.triangulation(config_dict)
        PoseAnalysis.filtering(config_dict)
        # PoseAnalysis.markerAugmentation(config_dict) # Marker augmentation requires markers that are not provided by RTMO: ['RHeel', 'RBigToe', 'RSmallToe', 'LSmallToe', 'LHeel', 'LBigToe']
        PoseAnalysis.kinematics(config_dict)
        ...
        config_dict.get('kinematics').update({'use_simple_model':True})
        PoseAnalysis.runAll(config_dict, do_synchronization=False, do_markerAugmentation=False, do_filtering=False)
        ...
        PoseAnalysis.runAll(do_synchronization=False, do_filtering=False)
