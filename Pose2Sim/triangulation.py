...
# [PATCH] 일부 calibration.toml이 TOML 스펙(배열은 homogeneous) 위반으로 toml 파서에서
# "Not a homogeneous array" 에러가 날 수 있습니다 (예: [1596.5, 0, 957.6] 처럼 int/float 혼합).
# 이 경우 자동으로 "숫자 배열 안의 정수"를 float(예: 0 -> 0.0)로 변환한 임시 파일을 만들어 사용합니다.
def _ensure_homogeneous_toml(calib_file: str) -> str:
    try:
        toml.load(calib_file)
        return calib_file
    except Exception as e:
        msg = str(e)
        if "Not a homogeneous array" not in msg:
            raise

    # Overwrite the original file in-place to avoid creating a separate *_fixed.toml
    fixed_path = calib_file
    try:
    ...
    # 배열 안의 정수 토큰을 float로 변환 (0 -> 0.0, 1 -> 1.0, -2 -> -2.0)
    # 문자열/키/테이블명에는 영향 없음.
    def _repl(m):
        tok = m.group(0)
        # 이미 float/지수표기면 유지
    ...
    # [] 안의 숫자만 대충 잡는 휴리스틱: 구분자(= [ , 공백)로 둘러싸인 정수
    s2 = re.sub(r"(?:(?<=\[)|(?<=,)|(?<=\s))(-?\d+)(?=(?:\s|,|\]))", _repl, s)
    ...
    # 다시 검증
    toml.load(fixed_path)
    logging.warning(f"[Triangulation] Calibration TOML was not homogeneous. Rewriting in-place with float tokens: {fixed_path}")
    return fixed_path
...
def recap_triangulate(config_dict, error, nb_cams_excluded, keypoints_names, cam_excluded_count, interp_frames, non_interp_frames, f_range_trimmed, f_range, trc_paths):
    ...
    # Read config_dict
    project_dir = config_dict.get('project').get('project_dir')
    # Resolve calibration file (GUI/Config.toml-free)
    calib_file_from_ui = (config_dict.get('paths', {}) or {}).get('calibration_file')
    if calib_file_from_ui and os.path.isfile(calib_file_from_ui):
        calib_file = calib_file_from_ui
        calib_dir = os.path.dirname(calib_file_from_ui)
    else:
        # Pose2Sim legacy: search for a 'calib*' folder next to the trial/session
        session_dir = os.path.realpath(os.path.join(project_dir, '..'))
        if not os.path.isdir(session_dir):
            session_dir = os.path.realpath(project_dir)
        try:
            if 'Config.toml' not in os.listdir(session_dir):
                session_dir = os.path.realpath(project_dir)
        except Exception:
            session_dir = os.path.realpath(project_dir)
        calib_dirs = [os.path.join(session_dir, c) for c in os.listdir(session_dir)
                     if os.path.isdir(os.path.join(session_dir, c)) and 'calib' in c.lower()]
        if len(calib_dirs) == 0:
            raise FileNotFoundError("No calibration folder found. Provide config_dict['paths']['calibration_file'] (toml).")
        calib_dir = calib_dirs[0]
        calib_files = glob.glob(os.path.join(calib_dir, '*.toml'))
        if len(calib_files) == 0:
            raise FileNotFoundError(f'No .toml calibration file found in: {calib_dir}')
        calib_file = max(calib_files, key=os.path.getctime)  # latest calibration file
...
def triangulate_all(config_dict):
    ...
    # Read config_dict
    project_dir = config_dict.get('project').get('project_dir')
    # Resolve session_dir robustly (Config.toml-free)
    session_dir = os.path.realpath(os.path.join(project_dir, '..'))
    if not os.path.isdir(session_dir):
        session_dir = os.path.realpath(project_dir)
    try:
    ...
    # Prefer explicit calibration file from GUI
    calib_file_from_ui = (config_dict.get('paths', {}) or {}).get('calibration_file')
    if calib_file_from_ui and os.path.isfile(calib_file_from_ui):
        calib_file = calib_file_from_ui
        calib_dir = os.path.dirname(calib_file_from_ui)
        ...
        try:
            ...
            raise Exception("No calibration directory found. Provide config_dict['paths']['calibration_file'] (toml).")
        try:
            calib_files = glob.glob(os.path.join(calib_dir, '*.toml'))
            calib_file = max(calib_files, key=os.path.getctime)  # latest calibration file
        except Exception:
    ...
    calib_file = _ensure_homogeneous_toml(calib_file)
    ...
    try:
        ...
        raise ValueError(f'No json files found in {pose_dir} subdirectories. Make sure you run PoseAnalysis.poseEstimation() first.')
    ...
    try: 
        ...
        try: 
            ...
            try:
                ...
                raise Exception(f'No json files found in {pose_dir}, {poseSync_dir}, nor {poseTracked_dir} subdirectories. Make sure you run PoseAnalysis.poseEstimation() first.')
